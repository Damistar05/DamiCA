...
dfrm5 = pd.DataFrame(myntra['seller_state'].value_counts())
dfrm5 = dfrm5.reset_index()
dfrm5 = dfrm5.rename(columns = {'index':'Seller_State','seller_state':'Count'})

dfrm6 = pd.DataFrame(myntra['seller_city'].value_counts(ascending=False)[:10])
dfrm6 = dfrm6.reset_index()
dfrm6 = dfrm6.rename(columns = {'seller_city':'Count','index':'Seller_City'})
...
dfrm7 = pd.DataFrame(myntra.groupby('seller_state')['brand_name'].value_counts())
dfrm7 = dfrm7.rename(columns={'brand_name':'Count'})
dfrm7 = dfrm7.reset_index()

stateName = list(dfrm7['seller_state'].unique())

dfrm8 = pd.DataFrame(myntra.groupby(['seller_state','seller_city'])['tags'].value_counts())
dfrm8 = dfrm8.rename(columns={'tags':'Count'})
dfrm8 = dfrm8.reset_index()
                            ...
                            html.Div([
                                dbc.Row([
                                    ...
                                    dbc.Col([
                                        ...
                                        dbc.Table.from_dataframe(dfrm6, striped=True, bordered=True, hover=True, className='table-success')
                                    ], width=6)
...
def generated_chart2(state):
    if not state:
        dash.no_update
    mask3 = dfrm7.seller_state.isin(state)
    mask4 = dfrm8.seller_state.isin(state)

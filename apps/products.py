...
dfrm1 = myntra.groupby(['tags', 'product_tag']).size().reset_index(name='count')
t1 = [{'label': tag, 'value': tag} for tag in dfrm1['tags'].unique()]
...
def generate_chart(tags):
    if not tags:
        dash.no_update
    filtered_df = dfrm1[dfrm1['tags'] == tags]
...
def generate_chart1(gender):
    if not gender:
        dash.no_update
    dfrm2 = myntra[myntra['rating'] != 0]
    mask = dfrm2.suitable_for.isin(gender)
    fig3 = px.scatter(dfrm2[mask], x="rating", y="rating_count", size='discount_percent', hover_data=['brand_name','tags','product_tag'])
    ...
    dfrm3 = myntra.groupby(['tags','suitable_for'])['marked_price'].mean()
    dfrm3 = dfrm3.reset_index()
    dfrm4 = myntra.groupby(['tags','suitable_for'])['discounted_price'].mean()
    dfrm4 = dfrm4.reset_index()
    mask1 = dfrm4.suitable_for.isin(gender)
    mask2 = dfrm3.suitable_for.isin(gender)
    fig4 = go.Figure(layout=go.Layout(paper_bgcolor="#95A5A6",plot_bgcolor='#95A5A6'))
    fig4.add_trace(go.Bar(x=dfrm4[mask1]['tags'], y=dfrm4['discounted_price'], name='Discounted Price'))
    fig4.add_trace(go.Bar(x=dfrm3[mask2]['tags'],y=dfrm3['marked_price'], name = 'Marked Price'))
